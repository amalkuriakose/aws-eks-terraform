...
resource "aws_subnet" "public_subnets" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name                     = "${local.project_name}-public-sn-${each.value}"
...
resource "aws_subnet" "private_subnets" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name                              = "${local.project_name}-private-sn-${each.value}"
...
resource "aws_default_network_acl" "default_nacl" {
  ...
  subnet_ids = concat([for item in aws_subnet.public_subnets : item.id], [for item in aws_subnet.private_subnets : item.id])
